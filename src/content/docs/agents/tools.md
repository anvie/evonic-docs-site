...
The `send_agent_message` tool supports injected system variables: pass key-value pairs via `injected_system_vars` that expand `` placeholders in the target's system prompt.
